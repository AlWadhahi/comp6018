...
class CourseRegistrationServicer(
    course_registeration_pb2_grpc.CourseRegistrationServicer
):
    def __init__(self):
    ...
    def RegisterStudent(self, request, context):
        ...
        if request.course_code in self.student_registrations:
            if request.student_id in self.student_registrations[request.course_code]:
                return course_registeration_pb2.RegisterStudentResponse(
                    success=False, message="Student already registered in this course."
                )

        self.student_registrations.setdefault(request.course_code, {})[
            request.student_id
    ...
    def ListCourses(self, request, context):
        courses = [
            course_registeration_pb2.Course(course_code=code, course_title=title)
    ...
    def ListStudents(self, request, context):
        ...
        students = [
            course_registeration_pb2.Student(student_id=id_, student_name=name)
            for id_, name in self.student_registrations.get(
                request.course_code, {}
            ).items()
        ]
        return course_registeration_pb2.ListStudentsResponse(
            students=students, success=True, message="Students listed successfully."
        )
